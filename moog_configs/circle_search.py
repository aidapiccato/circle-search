...
_WHITE = [1.0, 0, 1.0]
_TURQUOISE = [0.27, 1, .8]
_COLORS = [_RED, _GREEN, _BLUE, _YELLOW, _PURPLE, _PINK, _WHITE, _TURQUOISE]
_GRAY = [0, 0, 0.7]
_N_CIRCLES = 18
...
class StateInitialization:
    ...
    def __init__(self, max_attempts=np.inf, n_colors=None):
        ...
        def _fruit_generator(fruit_colors, target_color, r=0.35):
            n_circles = len(fruit_colors)
            fruit_factors = {'shape': 'circle', 'scale': 0.05}
            fruits_rad = np.linspace(0, 2 * np.pi - (2 * np.pi / n_circles), n_circles)
    ...
    def state_initializer(self):
        ...
        n_colors = np.random.choice(self._n_colors, size=1)
        n_items = np.random.randint(low=8, high=32, size=1)
        n_regions = np.random.randint(low=n_colors, high=n_colors+4, size=1)[0]
        frac_target = np.random.uniform(low=0.1, high=0.5, size=1)[0]
        # Generating task object
        task = CircleSearch(n_colors=n_colors, n_items=n_items, n_regions=n_regions, frac_target=frac_target,
                            n_attempts=self._max_attempts).task_dict()
        ...
        fruit_colors_map = [colors_shuffled[color] for color in task['map_circle']]
        map_fruits = self._fruit_generator(fruit_colors_map, target_color, r=0.15)

        # Generating fruits
        fruit_colors_occ = [colors_shuffled[color] for color in task['occ_circle']]
        fruits = self._fruit_generator(fruit_colors_occ, target_color, r=0.25)
...
def get_config(level, max_attempts=-1):
    ...
    screen_phase = game_rules.Phase(one_time_rules=(show_cue, hide_fruits, hide_agent, hide_map_fruits,
                                                    update_meta_state_screen), duration=40,
                                    name='screen')
    ...
    search_phase = game_rules.Phase(one_time_rules=(hide_cue, show_fruits, show_agent, show_map_fruits, occlude_fruits,
