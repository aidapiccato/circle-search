...
class StateInitialization:
    ...
    def state_initializer(self):
        ...
        n_colors = np.random.choice([3, 4])
        n_items = np.random.choice([3, 4])
        # Generating task object
        task = CircleSearchAmbiguousFlat(n_colors=n_colors, n_items=n_items, n_attempts=self._max_attempts).task_dict()
